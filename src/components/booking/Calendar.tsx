...
export function Calendar({ serviceId, onSelectSlot }: CalendarProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <div className="flex-1 bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <button onClick={handlePrevWeek} className="p-2 hover:bg-gray-100 rounded-full transition-colors">&larr;</button>
            <button onClick={handleNextWeek} className="p-2 hover:bg-gray-100 rounded-full transition-colors">&rarr;</button>
          </div>
        </div>
      ...
      {/* Time Slots */}
      <div className="flex-1 bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
        <h3 className="font-semibold text-gray-800 uppercase tracking-wider text-sm mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          {loading ? (
            ...
            <div className="col-span-2 text-center text-sm text-gray-500 py-4">No slots available</div>
          ) : (
            availableSlots.map((time) => (
              <button
                ...
                className={cn(
                  "py-3 px-4 rounded-md text-sm font-medium transition-all border",
                  selectedTime === time
