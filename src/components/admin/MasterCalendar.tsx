...
export function MasterCalendar({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="p-4 sm:p-6 border-b border-gray-200 flex flex-col sm:flex-row gap-3 sm:gap-0 justify-between sm:items-center">
        <h2 className="text-lg sm:text-xl font-semibold uppercase tracking-wider text-gray-800">
          {format(currentDate, "MMMM yyyy")}
        </h2>
        <div className="flex gap-2">
          <button onClick={handlePrevWeek} className="flex-1 sm:flex-none px-3 py-2 text-sm hover:bg-gray-100 rounded-md transition-colors border border-gray-200">&larr; Prev</button>
          <button onClick={handleNextWeek} className="flex-1 sm:flex-none px-3 py-2 text-sm hover:bg-gray-100 rounded-md transition-colors border border-gray-200">Next &rarr;</button>
        </div>
      </div>

      {/* Horizontal scroll wrapper so the 7-day grid stays usable on phones. */}
      <div className="overflow-x-auto">
        <div className="grid grid-cols-7 divide-x divide-gray-200 min-w-[700px]">
          {weekDays.map((day, i) => {
      ...
      {/* Tiny hint visible only on small screens. */}
      <p className="md:hidden text-[11px] text-gray-400 text-center py-2 border-t border-gray-100">
        Swipe horizontally to see all days &rarr;
      </p>
    </div>
  );
}
