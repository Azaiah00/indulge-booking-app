    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 sm:mb-10 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-light tracking-tight text-[var(--color-foreground)] mb-2 uppercase">
            Welcome, <span className="italic font-serif text-[var(--color-primary)]">{session.user.name || "Guest"}</span>
          </h1>
          <p className="text-gray-500 text-sm sm:text-base">Manage your appointments and preferences.</p>
        </div>
        <Link href="/book" className="w-full sm:w-auto">
          <Button variant="primary" size="md" className="w-full sm:w-auto">Book Appointment</Button>
        </Link>
      </div>

      <h2 className="text-base sm:text-xl font-semibold uppercase tracking-widest text-gray-800 mb-4 sm:mb-6">Upcoming Appointments</h2>
      ...
      <div className="space-y-4">
        {upcoming.length === 0 ? (
          ...
          upcoming.map((app) => (
            <Card key={app.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 sm:p-6 gap-3 sm:gap-0">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-1">
                  <h3 className="text-base sm:text-lg font-medium text-gray-900">{app.service?.name ?? "Service"}</h3>
      ...
      <h2 className="text-base sm:text-xl font-semibold uppercase tracking-widest text-gray-800 mt-12 sm:mt-16 mb-4 sm:mb-6">Past Services</h2>
      <div className="space-y-4">
        {past.length === 0 ? (
          ...
          past.map((app) => (
            <Card key={app.id} className="p-4 bg-gray-50 border-transparent">
              <div className="flex justify-between items-center opacity-70 gap-3">
                <div className="min-w-0">
                  <h3 className="font-medium text-gray-900 truncate">{app.service?.name ?? "Service"}</h3>
